...
@RequestMapping("/developer") //统一加前缀
...
/*    @Autowired
     ...
     */
    @PostMapping("/login")
    public Result login(@RequestBody Developer developer){ //requestBody 将前端传来的数据转换成java对象
        String userId = developer.getUserId();
        String password = developer.getPassword();
        Integer role = developer.getRole();
        //后端再次判断参数是否为空
        ...
        return Result.success(developerService.login(developer));
    }
    ...
    public Result register(@RequestBody Developer user){
        String userId = user.getUserId();
        String password = user.getPassword();
        if(StrUtil.isBlank(userId)||StrUtil.isBlank(password)){
            return Result.error(Constants.CODE_400,"参数错误");
        }
        developerService.register(user);
...
/*    @GetMapping("/selectById/{userId}")
    public Result selectById(@PathVariable Integer userId){
        Developer user = developerService.getById(userId);
